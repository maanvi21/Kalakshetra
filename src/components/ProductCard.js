...
export default function ProductCard({ item }) {
  ...
  }, [wishlistDispatch, isItemInWishlist, user, navigate]);
  ...
  const handleAddToCart = useCallback((item) => {
    ...
    if (existingItem) {
      cartDispatch({ type: 'UPDATE_QUANTITY', _id: item._id, quantity: existingItem.quantity + 1 });
    } else {
    ...
    <div className="product-card-container">
      <div className="prod_container" key={item._id}>
        <div className="prod_image-wrapper">
          <img src={item.image1 || '/placeholder.png'} alt={item.alt || item.title || 'Product'} />
        </div>
